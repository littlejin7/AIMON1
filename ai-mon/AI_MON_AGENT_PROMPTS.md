...
```
작업: 이미 완성된 프론트에 미션 위젯 1개를 "추가"만 한다. (프론트 재작성 아님)
- [중요] 기존 UI/페이지/컴포넌트를 새로 만들거나 갈아끼우지 말 것. 기존 화면은 그대로 두고
  미션 위젯만 신규 컴포넌트로 추가. 기존 컴포넌트(game_rewards 등)는 패턴 참고/재사용만.
- 먼저 기존 프론트 구조(홈 대시보드 위치, game_rewards 위젯, api 클라이언트)를 읽고
  "어디에 무엇을 추가할지"를 보고한 뒤 진행.
- GET /missions 결과로 진척바 + 수령 버튼 렌더. 데일리/위클리 탭 또는 섹션 구분.
완료 기준: 기존 화면 회귀 없이 미션 위젯이 추가되고, 수령 버튼이 POST /missions/claim를 호출.
