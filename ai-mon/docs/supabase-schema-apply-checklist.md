...
서버 환경에서 `USE_SUPABASE=true`로 전환한 후, 다음 6가지 시나리오에 대해 Smoke Test를 진행하여 정상 연동 여부를 확인합니다.

```mermaid
graph TD
    A[1. 회원가입/로그인] --> B[2. 레벨 테스트 수행]
    B --> C[3. 개념 퀴즈 완료]
    C --> D[4. 미니보스 클리어]
    D --> E[5. 테마 상점 이용 및 미션 보상 수령]
    E --> F[6. 회원 탈퇴 및 동일 ID 재가입]
```

### [Test 1] 회원가입 및 로그인
1. 신규 사용자로 가입을 시도합니다.
2. 가입한 정보로 로그인을 진행하고 Access Token 및 Refresh Token이 발급되는지 확인합니다.
3. 데이터베이스 `users` 테이블에 신규 UUID 레코드가 정상적으로 적재되었는지 확인합니다.

### [Test 2] 레벨 테스트 수행
1. 로그인 후 첫 진입 시 레벨 테스트를 수행합니다.
2. 테스트 결과를 정상적으로 전송하고 결과 페이지가 표시되는지 확인합니다.
3. DB의 `users.is_level_tested`가 `true`로 업데이트되었는지 확인합니다.

### [Test 3] 개념 퀴즈 풀이 및 채점
1. 특정 유닛의 스테이지에 진입하여 개념 퀴즈를 풉니다.
2. 정오답 채점 결과 및 피드백이 정상적으로 화면에 노출되는지 확인합니다.
3. DB의 `attempts` 테이블에 각 문제 풀이 기록이 실시간으로 Insert 되는지 확인합니다.

### [Test 4] 미니보스 도전 및 클리어
1. 개념 퀴즈 클리어 후 연이어 등장하는 미니보스 배틀을 시작합니다.
2. 정답/오답에 따라 화면의 체력(HP) 게이지가 동기화되어 소모되는지 확인합니다.
3. 미니보스 클리어 성공 시, DB의 `progress` 테이블에 성공 여부와 스코어가 업데이트되는지 확인합니다.

### [Test 5] 테마 구매 및 미션 보상 수령 (동시성 CAS 검증)
1. 퀴즈와 보스 격파로 획득한 XP를 이용하여 상점에서 테마를 구매합니다.
2. `users.purchased_themes` 배열에 해당 테마가 추가되고 XP가 정상 차감되는지 확인합니다.
3. 미션 완료 페이지에서 보상을 수령(`POST /missions/claim`)하고 미션 정보가 `users.missions`에 올바르게 기록되는지 확인합니다.

### [Test 6] 회원 탈퇴 및 재가입
1. 설정 페이지에서 '회원 탈퇴(계정 삭제)'를 수행합니다.
2. `users` 테이블의 `deleted_at` 컬럼에 탈퇴 일시(Timestamp)가 기록되고 로그아웃 처리되는지 확인합니다.
3. **핵심 검증**: 방금 탈퇴한 동일한 `username`과 `email`로 즉시 재가입을 시도합니다. 가입이 성공하고 새로운 사용자 UUID로 정상 작동하는지 확인합니다.
