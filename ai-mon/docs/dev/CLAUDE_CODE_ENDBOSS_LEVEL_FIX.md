...
| **B-2. 엔드보스 난이도 = course_level 단일 커플링** | 🚫 제외 확정(불필요) | 재도전/레벨선택 UX는 **기획상 불필요**로 결정(2026-07-05). course_level 커플링 유지가 의도된 동작. 코드 변경 안 함. |
| **C. 유닛 진행 기반 승격 부재** | 🚫 제외 확정(정책 A) | 승격은 **엔드보스 클리어로만**(`endboss.py:526`). 유닛 8 완주 승격은 안 하기로 결정(2026-07-05). boss.py 변경 안 함. |

**결론(1줄):** 원래 신고된 "엔드보스가 초급/중급/고급 문제를 이상하게 냄"의 두 확정 원인(레벨테스트 500, 이력 오염 승격)은 **이미 코드에 반영·테스트 그린.** B-2·C 는 기획 결정으로 **하지 않음** → **남은 코드 작업 없음, 바로 배포 단계.**
...
## 2. ~~(선택) 이슈 B-2 — 엔드보스 난이도 디커플링~~ 🚫 하지 않음 (2026-07-05 확정)

> **결정: 진행 안 함.** "상위 레벨 유저의 하위 엔드보스 재도전" / "도전 레벨 직접 선택"은 기획상 불필요. course_level 커플링 유지가 의도된 동작이라 코드 변경 없음. 아래 블록은 참고용(향후 요구 생기면 그때 판단), **지금 실행 대상 아님.**
...
## 3. ~~(선택) 이슈 C — 유닛 진행 기반 승격~~ 🚫 하지 않음 (정책 A 확정)

> **결정: 진행 안 함.** 승격은 엔드보스 클리어로만. 아래 블록은 참고용, 실행 대상 아님.
...
## 4. ~~(B-2 적용 시) 프론트 — target_level 전달 + Phase3 터미널~~ 🚫 하지 않음

> B-2 를 하지 않으므로 프론트 target_level 배선도 불필요. (Phase3 CodeInput 터미널은 이미 동작 중 — 신규 작업 없음.)
...
- progress 기록만 상위레벨 → 언락 ["beginner"] (오염 비기여)
※ B-2/C 미진행이므로 target_level·유닛승격 관련 케이스는 없음.
```
...
- 남은 **코드 작업 없음** → 이 문서 기준 추가 모델 작업 불필요.
- (참고) 이후 배포 설정 파일 생성 등 기계적 작업은 `claude-sonnet-5`, 구조 판단 필요 시 `claude-opus-4-8`.
...
- **B-2(target_level 디커플링) 하지 않음(제외 확정).** 재도전/레벨선택 UX는 기획상 불필요로 결정. course_level 커플링 유지가 의도된 동작이라 코드 변경 없음. (defer 아님 — 현 기획에선 불필요.)
- 따라서 남은 코드 작업 **없음.** 이슈 A·B-1 은 이미 반영·테스트 그린(11 passed). **다음은 배포뿐.**
