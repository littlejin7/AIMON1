...
# ---------------------------------------------------------------------------
# 유저 상태(도메인) mutation 은 routers.user_state 로 분리했다. 파일 하단에서
# import 하는 이유: user_state 가 serialize_user/current_week_ranking_score 에서
# routers.utils(=이 모듈)의 iso_week / get_progress_by_user 를 call-time 로
# 역참조하므로, 이 모듈이 그 이름들을 먼저 정의한 뒤 import 해야 순환이 안전하다.
# 외부는 계속 `from routers.utils import serialize_user, apply_xp, ...` 로 접근한다.
# ---------------------------------------------------------------------------
