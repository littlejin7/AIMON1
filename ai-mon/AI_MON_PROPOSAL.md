...
### 보스 도전
```
스테이지 1-1 ~ 1-7 전부 완료 → 유닛 보스 해금 → 도전
```
- 하루 2번 무료 도전
...
- beginner 유닛 1~7: `output_select` ×4, `fill_in_blank` ×4, `error_find` ×2 / easy×2 + medium×4 + hard×4
- beginner 유닛 8 (AI 에이전트): `multiple_choice` ×4, `fill_in_blank` ×4, `output_select` ×2 (error_find 미사용)
- intermediate/advanced: 추후 결정
...
| 유닛 보스 | output_select + fill_in_blank + error_find | fill_in_blank + code_input | code_input 위주 |
...
- Unit 1: stage_quiz 98개 (Set A/B 각 7개×7스테이지) + miniboss 91개 + unit_boss beginner 10개
- Unit 2~8: unit_boss beginner 각 10개 (총 70개) / stage_quiz·miniboss 제작 예정
- intermediate·advanced unit_boss: 각 2개 (Unit 1 한정, 나머지 제작 예정)
